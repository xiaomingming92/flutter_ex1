...
class TokenManager {
  ...
  static Future<void> setTokens(LoginRes loginResData) async {
    ...
    await Future.wait([
      ...
      _storage.write(
        ...
        value: data['refreshExpiresAt'].toString(),
      ),
  ...
  static Future<bool> isTokenExpired() async {
    ...
    // 解析token过期时间
    try {
      final refreshExpiresAt = await _storage
          .read(key: _refreshExpiresAtKey)
          .then(
            (value) => int.parse(
  ...
  static Future<int?> getRefreshExpiresAt() async {
    final refreshExpiresAt = await _storage.read(key: _refreshExpiresAtKey);
    if (refreshExpiresAt == null) {
      return null;
    }
    return int.parse(refreshExpiresAt);
  }
}
